...
\begin{enumerate}
    ...
    \item \textbf{值函数\footnote{说人话就是从当前状态出发，未来还能有多大长期收益}与贝尔曼方程\footnote{值函数是未知目标，贝尔曼方程是一个递归式子，比如状态值函数是衡量s状态的价值到其子状态s'之间的递归关系；}：}
    
    \textcolor{red}{感觉不太可能考这个吧？就算考把贝尔曼方程背下来就行，本质上就是跟着状态值函数更新图的Markov过程，纯粹就是高中条件概率题，别看符号多，其实很好理解}
    \begin{itemize}
        \item \textbf{状态值函数（state-value function）：}\footnote{现在我们衡量状态s和其子状态s'之间的递归关系。我们已知当前状态s和策略$\pi$，考虑从s状态出发，首先我们根据策略$\pi$能选择多个动作a，为了考虑所有动作带来的总的后续收益，我们需要对所有的动作带来的收益求和；继续看，现在我们进一步确定了s和a，那么我们就会以p转移到s'，并据此获得奖励r，但是这个s'到哪也要根据概率来，所以对于每个动作而言要计算后续回报，就要对所有s'能收益求和；那我们现在确定了s,a,s'，这一步单独拿到了奖励r，而s'的未来价值可以用s'的状态值函数衡量，递归到此结束，所以有两个求和。}
    ...
    \begin{figure}[H]
        ...
        \label{fig:placeholder}
    \end{figure}
        \item 根据状态值函数贝尔曼方程，可以构建状态值函数更新图。空心圆表示状态，实心圆表示动作。由图可知，状态值函数与动作值函数满足如下关系式：\footnote{现在我们衡量动作a和其到s'后的子动作a'之间的递归关系。我们已知当前状态s，动作a和策略$\pi$，考虑从s状态选择动作a出发，那么我们就会以p转移到s'，并据此获得奖励r，但是这个s'到哪也要根据概率来，所以对于每个动作而言要计算后续回报，就要对所有s'能收益求和；那我们现在确定了s,a,s'，这一步单独拿到了奖励r，而s'的未来价值可以用s'的状态值函数衡量，我们进一步用s'的状态值函数拆分为动作a'的动作值函数，每一个s'在策略$\pi$下对应多个动作a'，这些回报对a'应该求和，每个a'的回报就是其动作值函数，递归结束。}
        $$
