...
\begin{document}
\section{基础智能体框架、任务规划与大模型智能体}\label{sec:agent-framework}
\subsection{智能体理论框架}\label{sec:agent-theory}
\begin{enumerate}
    \item \textbf{智能体定义}
    \begin{itemize}
...
\end{table}
    ...
    \begin{itemize}
        ...
        \item \textbf{基于效用型：}利用效用函数把状态（状态序列）映射到实数，以描述智能体状态相关的程度，可以辅助进行决策；最佳期望效用是通过计算所有可能结果状态的加权平均得到，其权值由结果的概率确定
    \end{itemize}
...
\end{table}
    ...
    \end{itemize}
\end{enumerate}
\subsection{基于大模型的智能体}\label{sec:llm-agent}
\begin{enumerate}
    ...
    \end{itemize}
\end{enumerate}
\subsection{智能体任务规划}\label{sec:agent-task-planning}
\begin{enumerate}
    \item \textbf{任务规划定义}
    \begin{itemize}
    ...
    \item \textbf{任务规划分类}
      \begin{itemize}
            \item \textbf{任务分解：}\textcolor{red}{这个最重要，下面的什么符号规划器和神经规划器知道就行了}
            \begin{itemize}
