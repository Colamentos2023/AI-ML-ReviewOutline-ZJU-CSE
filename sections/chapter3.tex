...
\begin{document}
\section{神经网络}\label{sec:neural-network}

\subsection{感知器}\label{sec:perceptron}
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \]
        \begin{itemize}
            \item \textbf{收敛的前提条件：}训练样例线性可分\footnote{存在一条直线（二维）、一个平面（三维）或更一般地一个超平面（高维），能够把所有正例和反例完全分开。}，且使用了充分小的$\eta$。
        \end{itemize}
...
\subsection{非线性单元}\label{sec:nonlinear-unit}
\begin{enumerate}
    \begin{figure}[H]
        \centering
    ...
    \end{figure}
    \item \textbf{多层网络}：能够表示种类繁多的非线性曲面，由单元来连接，要求该单元：
    \begin{itemize}
        \item 输出是输入的非线性函数
        \item 输出是输入的可微函数
    \end{itemize}
    ...
    \end{itemize}
\end{enumerate}
\subsection{反向传播算法}\label{sec:backprop}
\begin{enumerate}
...
\end{enumerate}
\subsection{前馈网络的表征能力}\label{sec:ffn-capacity}
\begin{enumerate}
    ...
    \item \textbf{任意函数}：任意函数可以被一个有\textbf{三层}单元的网络以任意精度\textbf{逼近}。两个隐藏层使用sigmoid单元，输出层使用线性单元，每层所需单元数不确定。
\end{enumerate}
...
\end{enumerate}
\subsection{泛化、过拟合与停止判据}\label{sec:nn-generalization}
\begin{enumerate}
