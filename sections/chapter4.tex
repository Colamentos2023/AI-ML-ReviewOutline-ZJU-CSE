...
\begin{document}
\section{决策树学习}\label{sec:decision-tree}
...
\subsection{ID3算法}\label{sec:id3}
\begin{enumerate}
...
\end{enumerate}
\subsection{假设空间和归纳偏置}\label{sec:dt-inductive-bias}
是指决策树学习的假设空间和归纳偏置。
\begin{enumerate}
...
\end{enumerate}
\subsection{过拟合}\label{sec:dt-overfitting}
\begin{enumerate}
    \item \textbf{原因：}
    \begin{enumerate}
        \item 训练样例含有随机错误或噪声
...
\subsection{ID3算法的扩展}\label{sec:dt-extensions}
\begin{enumerate}
    \item \textbf{原因：}
    \begin{itemize}
        \item ID3 只能处理离散值属性；
