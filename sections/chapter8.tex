...
\begin{document}
\section{线性模型}\label{sec:linear-model}
线性模型一般形式
...
\subsection{线性回归（linear regression）}\label{sec:linear-regression}
\begin{enumerate}
...
\textcolor{red}{这里还是看一下L2和L1正则化，也就是岭回归和Lasso 回归的对比，这里大概不会出公式，但是有可能考个概念题。}
\end{enumerate}
...
\subsection{线性分类}\label{sec:linear-classification}
...
\begin{enumerate}
    ...
    \end{itemize}
\end{enumerate}
\subsection{关键问题扩展}\label{sec:linear-key-issues}
\begin{enumerate}
    \item \textbf{过拟合阶数选择}
    \begin{enumerate}
